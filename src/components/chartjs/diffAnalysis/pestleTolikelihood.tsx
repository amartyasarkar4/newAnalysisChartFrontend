...
import PolarAreaChart from "../alltypes/PolarArea";
// ExamplePage.js

const PestleTolikelihood = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://newsanalysischartbackend-1.onrender.com/api/v1/amartya/pestle_to_avgLikelihood"
        );
        if (!response.ok) {
  ...
  return (
    <div>
      <h1 className="mt-8 text-md font-semibold">
        PESTLE WISE AVERAGE LIKELIHOOD
      </h1>
      <PolarAreaChart data={data} minWidth={"400px"} minHeight={"500px"} />
    </div>
  );
};

export default PestleTolikelihood;
